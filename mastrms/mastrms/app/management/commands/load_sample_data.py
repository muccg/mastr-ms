...
class Command(NoArgsCommand):
    ...
    def handle(self, *args, **options):
        ...
        nc, created = NodeClient.objects.get_or_create(
            organisation_name="org", site_name="site", station_name="station")

        rg, created = RuleGenerator.objects.get_or_create(name="Empty Rule Generator", description="Rule generator for testing",
                                                          state=RuleGenerator.STATE_ENABLED, accessibility=RuleGenerator.ACCESSIBILITY_ALL, version=1, created_by=admin)

        proj, created = Project.objects.get_or_create(
            title="Test Project", description="This project is for testing Mastr-MS", client=user)
        proj.managers.add(admin)

        ex, created = Experiment.objects.get_or_create(
            title="Test experiment", description="This experiment was created for test purposes.", comment="This is a comment about the experiment", status=ExperimentStatus.objects.get(
                name="Designed"), job_number="000", project=proj, instrument_method=InstrumentMethod.objects.first(), sample_preparation_notes="Example sample preparation notes")
...
def load_debug_data():
    ...
    m, _ = Spirit.objects.get_or_create(
        first_name="Wendy", last_name="Lorrimar", defaults={
            "second_name": "Maree", "maiden_name": "Styles", "dob": datetime.date(
                1956, 9, 23), "dob_checked": True, "sex": "F"})
    m.save()

    f, _ = Spirit.objects.get_or_create(
        first_name="Gary", last_name="Lorrimar", defaults={
            "second_name": "Wilfrid", "dob": datetime.date(
                1952, 3, 24), "dob_checked": True, "sex": "M"})
    f.save()
    ...
    gm2, _ = Spirit.objects.get_or_create(
        first_name="Margaret", last_name="Styles", defaults={
            "second_name": "Dorothy", "dob": datetime.date(
                1930, 1, 1), "dob_checked": False, "sex": "F"})
    gm2.save()
    gf2, _ = Spirit.objects.get_or_create(
        first_name="Basil", last_name="Styles", defaults={
            "second_name": "Hargrave", "dob": datetime.date(
                1921, 1, 1), "dob_checked": False, "dod": datetime.date(
                2011, 10, 1), "dod_checked": False, "sex": "M"})
    gf2.save()
    ...
    v, _ = Spirit.objects.get_or_create(
        first_name="Vicky", last_name="Styles", defaults={
            "mother": gm2, "father": gf2, "dob": datetime.date(
                1953, 6, 6), "dob_checked": False, "sex": "F"})
    v.save()

    l, _ = Spirit.objects.get_or_create(
        first_name="Lynley", last_name="Styles", defaults={
            "mother": gm2, "father": gf2, "dob": datetime.date(
                1958, 6, 6), "dob_checked": False, "sex": "F"})
    l.save()

    t, _ = User.objects.get_or_create(
        email="admin@example.com", defaults={
            "is_superuser": True, "is_staff": True, "first_name": "Another Admin", "last_name": "User", "second_name": "The"})
    t.first_name = "Admin"
    t.last_name = "User"
    t.save()
    ...
    brentwood = Suburb.objects.get(
        name__iexact="Brentwood",
        state__abbrev="WA",
        postcode="6153")
    ...
    address, _ = ContactDetails.objects.get_or_create(
        address_line1="Brentwood Village Medical Centre", suburb=brentwood, defaults={
            "address_line2": "67 Cranford Ave", "phone_work": "(08) 9316 8014", })
