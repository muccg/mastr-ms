...
def client_default_default_data_path():
    return getattr(settings, "REPO_FILES_ROOT", "")

def client_default_username():
    return getattr(settings, "CHMOD_USER", "")

def client_default_hostname():
...
class NodeClient(models.Model):
    ...
    default_data_path = models.CharField(max_length=255, default=client_default_default_data_path, help_text="File upload path on the Mastr-MS server")
    username = models.CharField(max_length=255, default=client_default_username, help_text="Username to use for the rsync command")
    hostname = models.CharField(max_length=255, default=client_default_hostname, help_text="rsync destination hostname")
