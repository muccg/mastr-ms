...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        If there are no InstrumentMethods, create one called "Default
        Method". It appears that instrument methods aren't used by
        Mastr-MS so it doesn't really matter what the object contains.
        """

        # First reapply repository migration 0002 because in the past
        # it was commented out.
        db.alter_column('repository_instrumentmethod', 'creator_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['users.User'], null=True))

        if orm.InstrumentMethod.objects.count() == 0:
            print "Creating a default instrument method"
            orm.InstrumentMethod.objects.create(
                title="Default Method",
                method_name="Default Method",
                creator=None,
                created_on=datetime.date.today(),
                method_path="D:\\mastrms",
                template="csv",
                version="1",
    ...
    def backwards(self, orm):
        "Don't really want to reverse this migration"
