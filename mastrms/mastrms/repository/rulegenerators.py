...
def listRuleGenerators(user=None, accessibility=False, showEnabledOnly=False):
    ...
    apply_accessibility_node = Q(
        accessibility=RuleGenerator.ACCESSIBILITY_NODE) & Q(
        node=usernode)
    apply_accessibility_user = Q(
        created_by=user) & Q(
        accessibility=RuleGenerator.ACCESSIBILITY_USER)
    ...
    # only bother doing accessibility if you arent an Admin or MA admin, and
    # accessibility is true.
    if accessibility and (user is not None) and not user.IsAdmin and not user.IsMastrAdmin:
        rows = rows.filter(
            apply_accessibility_node | apply_accessibility_user | apply_accessibility_everyone)
        ...
        if candidateRG.is_accessible_by(user):
            ...
            if kwargs.get('state') is not None:
                ...
                if candidateRG.created_by != user:
                    raise Exception(
                        "Only %s can change the enable or disable this rule generator." %
                        (candidateRG.created_by.email))
...
def convert_to_dict(rulegenerator):
    ...
    d['startblock'] = [{'count': r.count,
                        'component_id': r.component_id,
                        'component': r.component.sample_type} for r in rulegenerator.start_block_rules]
    d['sampleblock'] = [
        {
            'count': r.count,
            'component_id': r.component_id,
    ...
    d['endblock'] = [{'count': r.count,
                      'component_id': r.component_id,
                      'component': r.component.sample_type} for r in rulegenerator.end_block_rules]
    return d
